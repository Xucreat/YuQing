...
async function selectEvent(ev: PropagationEventSummary) {
  ...
  try {
    const { data } = await api.get<PropagationGraph>(/propagation/graph/)
...
async function handleRebuild() {
  ...
  try {
    const { data } = await api.post<PropagationRebuildResponse>(/propagation/rebuild/)
    ElMessage.success(传播链构建完成：创建  个节点)
...
pathlib.Path(r'C:\Users\Administrator\Desktop\YQ\frontend\src\views\Propagation.vue').write_text(propagation_vue, encoding='utf-8')
print('Propagation.vue created')
