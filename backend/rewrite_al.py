...
async function handleCollect() {
  ...
  try {
    const { data } = await api.post('/collector/run', {}, { timeout: 60000 })
...
al.write_text(content, 'utf-8')
print(f'Written: {al.stat().st_size} bytes')
v = al.read_text('utf-8')
lines = v.split('\n')
print(f'Lines: {len(lines)}')
print(f'Has nav: {v.count(\"router-link\")}')
print(f'Has brand: {\"舆情监测\" in v}')
print('First 5:')
