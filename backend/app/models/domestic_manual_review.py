...
class DomesticManualReview(Base):
    __tablename__ = "domestic_manual_reviews"
    __table_args__ = (
        CheckConstraint(
            "review_status IN ('pending_review','confirmed','rejected','superseded')",
            name="ck_domestic_manual_reviews_status",
        ),
        CheckConstraint(
            ...
            name="ck_domestic_manual_reviews_decision",
        ),
        CheckConstraint(
            "display_decision IS NULL OR display_decision IN ('keep_rule','use_ai_display')",
            name="ck_domestic_manual_reviews_display_decision",
        ),
        CheckConstraint(
            "event_review_status IN ('pending','confirmed','rejected')",
            name="ck_domestic_manual_reviews_event_status",
        ),
        CheckConstraint(
            "alert_review_status IN ('pending','confirmed','rejected')",
            name="ck_domestic_manual_reviews_alert_status",
        ),
        Index("ix_domestic_manual_reviews_status", "review_status"),
        Index("ix_domestic_manual_reviews_opinion", "opinion_id"),
        Index("ix_domestic_manual_reviews_ai_result", "ai_result_id"),
        Index("ix_domestic_manual_reviews_batch", "batch_run_id"),
    )

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    opinion_id: Mapped[int] = mapped_column(ForeignKey("opinions.id", ondelete="CASCADE"), nullable=False)
    ai_result_id: Mapped[int] = mapped_column(ForeignKey("domestic_ai_results.id", ondelete="CASCADE"), nullable=False)
