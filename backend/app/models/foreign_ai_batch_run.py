...
class ForeignAIBatchRun(Base):
    __tablename__ = "foreign_ai_batch_runs"
    __table_args__ = (
        Index("ix_foreign_ai_batch_runs_status", "status"),
        Index("ix_foreign_ai_batch_runs_created_at", "created_at"),
    )

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    run_id: Mapped[str] = mapped_column(String(64), unique=True, nullable=False)
    task_id: Mapped[str | None] = mapped_column(String(64), nullable=True)
    scope: Mapped[str] = mapped_column(String(16), nullable=False, default="count")
