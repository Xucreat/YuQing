...
onMounted(() => {
  if (collectStore.activeTaskId) startLive(collectStore.activeTaskId)
  reload()
})
  ...
  batchTimer = setTimeout(async () => {
    ...
    try {
      const { data: running } = await api.get<CollectionLogListResponse>(
        '/admin/data-sources/collection-logs', { params: { status: 'running', size: 100 } })
...
watch(() => collectStore.activeTaskId, (id) => {
  if (id && id !== liveTaskId.value) startLive(id)
})
