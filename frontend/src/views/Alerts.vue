  ...
  <div class="alerts" v-loading="loading">
    ...
    <template v-if="activeTab === 'rules'">
        ...
        <el-card v-if="scope === 'domestic'" shadow="never" class="table-card">
          <el-table :data="rules" stripe>
            ...
            <el-table-column label="规则类型" width="120"><template #default="{ row }">{{ row.rule_type === 'ai_risk_score' ? 'AI 风险分' : '规则风险分' }}</template></el-table-column>
    ...
    <template v-else-if="activeTab === 'records'">
        ...
        <el-card v-else shadow="never" class="filter-card">
          <span class="inline-switch"><el-switch v-model="hideForeignFalsePositive" @change="loadForeignRecords" />隐藏误报</span>
    ...
    <el-dialog v-model="foreignHandleDialogVisible" title="外网预警处置" width="min(480px, calc(100vw - 24px))"><el-form label-width="88px"><el-form-item label="处置状态"><el-select v-model="foreignHandleForm.disposition_status"><el-option label="待处理" value="pending" /><el-option label="处理中" value="processing" /><el-option label="已解决" value="resolved" /><el-option label="已忽略" value="ignored" /><el-option label="误报" value="false_positive" /></el-select></el-form-item><el-form-item label="处置备注"><el-input v-model="foreignHandleForm.note" type="textarea" /></el-form-item></el-form><template #footer><el-button @click="foreignHandleDialogVisible = false">取消</el-button><el-button type="primary" :loading="foreignHandling" @click="submitForeignHandle">确认处置</el-button></template></el-dialog>
...
const route = useRoute(); const notifier = useAlertNotifier(); const { hasPermission } = usePermission()
const activeTab = ref<'rules' | 'records'>('rules'); const scope = ref<'domestic' | 'foreign'>('domestic')
...
const canWriteAlert = computed(() => hasPermission('alerts:write')); const canEvaluateForeign = computed(() => hasPermission('foreign:alerts:evaluate')); const canForeignRuleWrite = computed(() => hasPermission('foreign:alerts:rules:write')); const canForeignRuleEnable = computed(() => hasPermission('foreign:alerts:enable')); const canForeignReviewConfirm = computed(() => hasPermission('foreign:alerts:review:confirm')); const canForeignReviewReject = computed(() => hasPermission('foreign:ai:review:reject')); const foreignEvaluating = ref(false)
...
const foreignAlerts = ref<any[]>([]); const foreignFilters = reactive({ severity: '', source: '' }); const foreignDateRange = ref<[string, string] | null>(null); const hideForeignFalsePositive = ref(true)
const domesticRuleDialog = ref(false); const domesticEditing = ref(false); const domesticId = ref<number | null>(null); const domesticForm = reactive({ name: '', description: '', rule_type: 'risk_score', risk_threshold: 70, keywords: '', sources: '', risk_level: 'high', enabled: true })
...
const foreignOpinionDetailVisible = ref(false); const foreignOpinionDetailId = ref<number | null>(null); const foreignHandleDialogVisible = ref(false); const foreignHandling = ref(false); const foreignHandlingId = ref<number | null>(null); const foreignHandleForm = reactive({ disposition_status: 'pending', note: '' })
const STATUS_TEXT: Record<string, string> = { pending: '待处理', processing: '处理中', resolved: '已解决', ignored: '已忽略', false_positive: '误报' }; const STATUS_TAG: Record<string, string> = { pending: 'danger', processing: 'warning', resolved: 'success', ignored: 'info', false_positive: 'info' }
const statusText = (v: string) => STATUS_TEXT[v] || v || '待处理'; const statusTag = (v: string) => STATUS_TAG[v] || 'info'; const formatTime = (v?: string | null) => v ? v.replace('T', ' ').slice(0, 19) : '-'
...
async function loadForeignRecords() { loading.value = true; try { const params: any = { page: 1, size: 100 }; params.disposition_filter = hideForeignFalsePositive.value ? 'hide_fp' : 'all'; if (foreignFilters.severity) params.severity = foreignFilters.severity; if (foreignFilters.source) params.source = foreignFilters.source; if (foreignDateRange.value?.[0]) params.triggered_from = foreignDateRange.value[0]; if (foreignDateRange.value?.[1]) params.triggered_to = foreignDateRange.value[1]; foreignAlerts.value = (await api.get('/foreign/alerts', { params })).data.items || [] } catch (e: any) { foreignAlerts.value = []; if (!isPermissionDenied(e)) ElMessage.error(e?.response?.data?.detail || '加载外网预警失败') } finally { loading.value = false } }
...
function openForeignHandleDialog(row: any) { foreignHandlingId.value = row.id; foreignHandleForm.disposition_status = row.disposition_status || 'pending'; foreignHandleForm.note = row.disposition_note || ''; foreignHandleDialogVisible.value = true }
async function submitForeignHandle() { if (foreignHandlingId.value == null) return; foreignHandling.value = true; try { const { data } = await api.put(`/foreign/alerts/${foreignHandlingId.value}/handle`, { disposition_status: foreignHandleForm.disposition_status, note: foreignHandleForm.note }); const idx = foreignAlerts.value.findIndex(item => item.id === foreignHandlingId.value); if (idx >= 0) { if (foreignHandleForm.disposition_status === 'false_positive' && hideForeignFalsePositive.value) foreignAlerts.value.splice(idx, 1); else foreignAlerts.value[idx] = data } foreignHandleDialogVisible.value = false; ElMessage.success('外网预警处置完成') } catch (e: any) { if (!isPermissionDenied(e)) ElMessage.error(e?.response?.data?.detail || '外网预警处置失败') } finally { foreignHandling.value = false } }
